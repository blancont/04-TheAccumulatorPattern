...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        sq = rg.Square(rg.Point(circle.center.x + (circle.radius * k), circle.center.y + (circle.radius * k)),
                       circle.radius * 2)
        sq.attach_to(window)
    window.render()
...
def run_test_draw_circles_from_rectangle():
    ...
    draw_circles_from_rectangle(4, 3, rec, window)
    rec = rg.Rectangle(rg.Point(600, 450), rg.Point(700, 500))
    rec.fill_color = 'blue'
    rec.outline_color = 'red'
    draw_circles_from_rectangle(2, 4, rec, window)
    window = rg.RoseWindow(500, 500)
    rec = rg.Rectangle(rg.Point(250, 250), rg.Point(300, 350))
    rec.fill_color = 'yellow'
    rec.outline_color = 'pink'
    draw_circles_from_rectangle(2, 3, rec, window)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    rad1 = (rectangle.corner_2.y - rectangle.corner_1.y) / 2
    for k in range(m):
        circ = rg.Circle(rg.Point((rectangle.corner_1.x - (rad1 * 2 * k)) - rad1, rectangle.corner_1.y + rad1), rad1)
        circ.fill_color = rectangle.fill_color
        circ.attach_to(window)
    rad2 = (rectangle.corner_2.x - rectangle.corner_1.x) / 2
    for k in range(n):
        circ = rg.Circle(rg.Point(rectangle.corner_1.x + rad2, rectangle.corner_1.y - (rad2 * 2 * k) - rad2), rad2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    corn11 = rectangle1.corner_1
    corn12 = rectangle1.corner_2
    corn21 = rectangle2.corner_1
    corn22 = rectangle2.corner_2
    cent1 = rg.Point(((corn12.x - corn11.x) / 2) + corn11.x,
                     ((corn12.y - corn11.y) / 2) + corn11.y)
    cent2 = rg.Point(((corn22.x - corn21.x) / 2) + corn21.x,
                     ((corn22.y - corn21.y) / 2) + corn21.y)
    xmov = cent1.x - corn11.x
    ymov = cent1.y - corn11.y

    if corn12.x > corn11.x:
        if corn12.y > corn11.y:
            for k in range(n):
                line = rg.Line(rg.Point(cent1.x - (xmov * k), cent1.y + (ymov * k)),
                               rg.Point(cent2.x - (xmov * k), cent2.y + (ymov * k)))
                line.thickness = 5
                if k % 2 == 1:
